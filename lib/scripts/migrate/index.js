...
const syncArtistsAndBands = async () => {
  ...
  const promises = _bluebird.default.map(result, item => {
    if (item.role === 'member') {
      // TODO get the list of labels and bands
    ...
    if (item.role === 'bands') {
      // TODO get the list of artists and labels
...
const syncTracks = async () => {
  ...
  const promises = _bluebird.default.map(result, async item => {
    ...
    if (item.role === 'label-owner') {
      user = await Label.findOne({
        id: item.artist_id
      });
    }

    if (item.role === 'bands') {
      user = await Band.findOne({
        id: item.artist_id
      });
    }
