...
const syncReleases = async () => {
  ...
  return _bluebird.default.map(result, item => {
    ...
    if (item.tags) {
      data.tags = item.tags.split(',').map(tag => tag.replace(/-|\s/g, '')).filter(Boolean);
    }
...
const syncTracks = async () => {
  ...
  return _bluebird.default.map(result, async item => {
    ...
    if (item.tags) {
      data.tags = (0, _decodeUriComponent.default)((0, _decodeUriComponent.default)(item.tags)).split(',').map(tag => tag.trim().toLowerCase().replace(/-|\s/g, '')).filter(Boolean);
    }
